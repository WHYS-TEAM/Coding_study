...
'''
현재 도시: 1
방문 상태(mask): 0b0011  (0번, 1번 방문함)

→ 이제 2번 도시를 방문할 수 있을까?

if mask & (1 << 2):   # 2번 방문 여부 체크
→ False니까 OK

→ 이동: 1 → 2 (비용 adj[1][2] = 7)
→ 방문 상태 갱신: mask | (1 << 2) = 0b0111
...
LOG = ceil(log2(N))  # 최대 올라갈 수 있는 조상 단계 수
...
###############################################################################
# 6. Kruskal : 최소 신장 트리(MST) – 간선 Greedy + Union-Find
###############################################################################
V, E = map(int, input().split())
...
###############################################################################
# 7. 위상 정렬 (Kahn) : 진입차수 0부터 제거 – DAG 정렬
###############################################################################
from collections import deque
...
###############################################################################
# 9. Lazy Propagation : 구간 업데이트 O(log N)
###############################################################################
lazy = [0]*(4*N)          # 지연 값 저장
